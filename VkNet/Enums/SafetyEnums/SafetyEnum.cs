...
// ReSharper disable once StaticFieldInGenericType
        ...
        protected static TFilter CreateFromMask( ulong mask )
        {
                 ...
                 != 0 )
            {
                throw new ArgumentException(
                    string.Format(
                        "Mask contains value(s) that not defined for type {0} (mask except known values: {1:x8})",
                        typeof( TFilter ).FullName,
                        PossibleValues.Select( pair => pair.Key )
                            .Where( key => ( mask & key ) != 0 )
                            .Aggregate( mask, ( current, @ulong ) => current - @ulong ) ), "mask" );
            }
        ...
        {
            return string.Join( ",",
                PossibleValues.Where( pair => ( pair.Key & this._mask ) != 0 ).Select( pair => pair.Value ).ToArray() );
        }
        ...
        public override bool Equals( object obj )
        {
            ...
            return this.Equals( (SafetyEnum<TFilter>)obj );
        }
